...
var serviceURL = "http://www.chocolateboutiquemotel.com/sistema/app/servicios/";
 ...
 function actualizarListadoHabitaciones() {
    ...
    var myVal = myApp.virtualList('.habitaciones-list', {
        ...
        '<div class="row" style="margin-top: -3px;">' +
          ...
          '<div class="row" style="margin-top: -35px;">' +
            ...
            '<div class="col-50 tablet-50" style="margin-top: 53px;"><img src="" alt="" title="" width="99%"/>' +
              '<a href="#" class="button button-small button-fill button-raised color-green link" @click="showToastCenter">Agregar a pedido</a>' +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>' ,
     searchAll: function (query, items) {
          ...
          for (var i = 0; i < items.length; i++) {
              if (items[i].Descripcion.toLowerCase().indexOf(query.toLowerCase().trim()) >= 0) foundItems.push(i);
              if (items[i].Codigo.toLowerCase().indexOf(query.toLowerCase().trim()) >= 0) foundItems.push(i);
          }
          return foundItems; 
        }  
    });
...
$$(document).on('pageInit', '.page[data-page="premios"]', function (e) {
  myApp.alert("Se va a cargar la lista de habitaciones");
  actualizarListadoHabitaciones();
});
