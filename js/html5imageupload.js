...
(function (window, $, undefined) {
	...
	$.html5imageupload = function html5imageupload(options, element) {
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="btn btn-info btn-edit" title="' + (this.options.editTitle || 'Editar') + '"><i class="glyphicon glyphicon-pencil"></i></div>'
		this.button.saving		= '<div class="btn btn-warning saving">' + (this.options.saveLabel || 'Guardando...') + ' <i class="glyphicon glyphicon-time"></i></div>';
		this.button.zoomin		= '<div class="btn btn-default btn-zoom-in" title="' + (this.options.zoominTitle || 'Acercar') + '"><i class="glyphicon glyphicon-resize-full"></i></div>';
		this.button.zoomout		= '<div class="btn btn-default btn-zoom-out" title="' + (this.options.zoomoutTitle || 'Alejar') + '"><i class="glyphicon glyphicon-resize-small"></i></div>';
		this.button.zoomreset	= '<div class="btn btn-default btn-zoom-reset" title="' + (this.options.zoomresetTitle || 'Tam. Completo') + '"><i class="glyphicon glyphicon-fullscreen"></i></div>';
...
//		this.button.rotateccw	= '<div class="btn btn-default btn-rotate-ccw" title="' + (this.options.ccwTitle || 'Rotate counter clockwise') + '"><i class="glyphicon glyphicon-share icon-flipped"></i></div>';
		this.button.cancel		= '<div class="btn btn-danger btn-cancel" title="' + (this.options.cancelTitle || 'Cancelar') + '"><i class="glyphicon glyphicon-remove"></i></div>';
		this.button.done		= '<div class="btn btn-success btn-ok" title="' + (this.options.okTitle || 'Ok') + '"><i class="glyphicon glyphicon-ok"></i></div>';
		this.button.del			= '<div class="btn btn-danger btn-del" title="' + (this.options.delTitle || 'Borrar') + '"><i class="glyphicon glyphicon-trash"></i></div>';
	...
	$.html5imageupload.prototype = {
		_init: function() {
			...
			if (! (window.FormData && ("upload" in ($.ajaxSettings.xhr())))) {
				$(element).empty().attr('class','').addClass('alert alert-danger').html('Lo sentimos.. navegador incompatible!');
				return false;
			}
